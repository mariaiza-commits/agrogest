...
export default function Custos({ onAddBtn }) {
  ...
  async function confirmarPagar() {
    ...
    setSaving(true)
    const { error } = await supabase.rpc('fn_pagar_custo',{p_custo_id:modalPagar.id,p_data_pagamento:pagData,p_conta_id:pagContaId})
  ...
  async function confirmarPagarMassa() {
    ...
    setSaving(true)
    for (const id of selecionados) await supabase.rpc('fn_pagar_custo',{p_custo_id:id,p_data_pagamento:pagData,p_conta_id:pagContaId})
    setSaving(false);setModalPagarMassa(false);setSelecionados([]);load()
  }
